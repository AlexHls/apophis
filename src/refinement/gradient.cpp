...
AmrTag PressureGradient(MeshBlockData<Real> *rc) {
  ...
  const auto threshold = pmb->packages.Get("Hydro")->Param<Real>(
      "refinement/threshold_pressure_gradient");
  ...
  if (pmb->pmy_mesh->ndim == 3) {
    ...
    pmb->par_reduce(
        "check refine: pressure gradient", kb.s - 1, kb.e + 1, jb.s - 1,
    ...
    int k = kb.s;
    pmb->par_reduce(
        "check refine: pressure gradient", jb.s - 1, jb.e + 1, ib.s - 1,
        ib.e + 1,
...
AmrTag VelocityGradient(MeshBlockData<Real> *rc) {
  ...
  const auto threshold = pmb->packages.Get("Hydro")->Param<Real>(
      "refinement/threshold_xyvelocity_gradient");
  ...
  pmb->par_reduce(
      "check refine: velocity gradient", kb.s, kb.e, jb.s - 1, jb.e + 1,
      ib.s - 1, ib.e + 1,
