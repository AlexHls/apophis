...
TaskStatus ReinitializeLset(std::shared_ptr<MeshData<Real>> &md,
  ...
  pmb->par_for(
      "ReinitializeLevelset", 0, cons_pack.GetDim(5) - 1, kl, ku, jl, ju,
      ib.s - 1, ib.e + 1,
          ...
          for (int ii = std::max(0, i - LSET_BORDER);
            ...
            for (int jj = std::max(0, j - LSET_BORDER);
              ...
              for (int kk = std::max(0, k - LSET_BORDER);
                ...
                dist =
                    std::min(dist, SQR(coords.Xc<X1DIR>(kk, jj, ii) - cr) +
            ...
            for (int ii = std::max(0, i - LSET_BORDER);
              ...
              for (int jj = std::max(0, j - LSET_BORDER);
                ...
                for (int kk = std::max(0, k - LSET_BORDER);
                     kk < std::min(nzdim, k + LSET_BORDER); kk++) {
                  dist = std::min(dist,
                                  SQR(coords.Xc<X1DIR>(kk, jj, ii) - cr) +
                                      SQR(coords.Xc<X2DIR>(kk, jj, ii) - cr) +
                                      SQR(coords.Xc<X3DIR>(kk, jj, ii) - cr));
                }
              }
