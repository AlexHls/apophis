...
template <> struct Riemann<Fluid::euler, RiemannSolver::lhllc> {
  ...
  Solve(parthenon::team_mbr_t const &member, const int k, const int j,
    ...
    parthenon::par_for_inner(member, il, iu, [&](const int i) {
      ...
      Real machl = std::sqrt(wli[IV1] * wli[IV1] + wli[IV2] * wli[IV2] +
                             wli[IV3] * wli[IV3]);
      Real machr = std::sqrt(wri[IV1] * wri[IV1] + wri[IV2] * wri[IV2] +
                             wri[IV3] * wri[IV3]);
      Real chi = std::max(machl, machr);
      Real phi = chi * (2.0 - chi);

      Real pstar = .5 * (wli[IPR] + wri[IPR]) -
                   .5 * (wri[IV1] - wli[IV1]) * rhoa * ca * phi;
